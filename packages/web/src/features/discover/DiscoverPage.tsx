    ...
    <section className="standalone-page">
      ...
      <div className="table-wrap">
        <table><thead><tr><th>名称</th><th>所在 Agent</th><th>检查结果</th><th /></tr></thead>
          <tbody>{items.map((item) => <tr key={item.name}><td className="strong">{item.name}</td><td>{item.agents.join("、")}</td><td>{item.classification === "conflict" ? <span className="agent warning"><WarningCircle />内容不一致</span> : <span className="agent healthy"><CheckCircle />可以纳入</span>}</td><td><button className="button compact" onClick={() => void api.add(item.name, item.agents[0])}>纳入管理</button></td></tr>)}</tbody>
        </table>
