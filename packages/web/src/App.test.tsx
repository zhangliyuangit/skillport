...
import type { DiscoveredSkill, OperationResult, SkillPortApi, SkillSummary } from "./api.js";
...
describe("SkillPort app", () => {
  ...
  it("shows local paths in Settings", async () => {
    const user = userEvent.setup();
    render(<App api={api()} />);
    await user.click(screen.getByRole("button", { name: "设置" }));
    expect(screen.getByDisplayValue("~/.skillport/skills")).toBeVisible();
    expect(screen.getByDisplayValue("~/.codex/skills")).toBeVisible();
    expect(screen.getByDisplayValue("~/.claude/skills")).toBeVisible();
  });
});
